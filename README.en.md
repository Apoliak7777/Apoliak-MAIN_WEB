...
- 🔁 **Live demo right in the hero** - a real demo runs in a frame next to the headline and six chips (hair salon, restaurant, car service, dental clinic, joinery, municipality) swap it without a page load; without JavaScript the chips are plain links to the demos. Every page load shows a different demo than the last one (the last pick is kept in `sessionStorage`). The industries then rotate on their own every 9 seconds — the timer is a CSS animation of a bar inside the gold pill under the active chip; it pauses on hover, on focus, while loading, off screen and in a background tab, and stops for good after the first chip click. The address in the frame bar types itself out and, with a mouse, the frame tilts towards the cursor.
...
- ✉️ **Direct contact instead of a form** - there is no form. Through `mailto:` a form would promise a send it can only deliver for a visitor with a configured mail client; anyone reading mail in a browser tab would click and get nothing. In its place stand the address in large type, buttons for e-mail, phone, WhatsApp and SMS, and a short list of what to put in the message. WhatsApp is a plain `wa.me` link with a pre-filled line in the page's language (an icon in the bottom bar on mobile), and the e-mail templates also ask where the visitor heard about me.
...
- 👁️ **Scroll reveal** - an `IntersectionObserver` adds the `.in` class to `.rv` elements and the `.pas` strip. The effect only engages once the inline script in the head sets the `js` class on `<html>`, so without JavaScript the content is simply visible; should `script.js` fail to load, a CSS fallback shows the content within three seconds.
- 🎬 **Motion without libraries** (block 56 in `styles.css`) - the hero headline rises word by word with a gold line drawn under the italic, the strip figures and package prices count up from zero in each language's own number format, list items, pricing cards, process steps and questions reveal one after another, cards glow under the cursor, solid buttons get a sheen, the Standard card has an orbiting gold outline, questions expand smoothly (`::details-content`) and the header shows a reading-progress bar and the section being read. Elements added by the script carry no text, so the language versions need no new translations.
- 🐢 **Respect for `prefers-reduced-motion`** - with the setting on, only cross-fades, counters and cursor effects remain; nothing slides, tilts or rotates on its own.
- 🔍 **SEO and sharing** - canonical URL, Open Graph and Twitter Card tags, a 1200 × 630 `og-image.png` (plus `og-image-cs.png`, `og-image-hu.png`, `og-image-pl.png`, `og-image-en.png`, `og-image-de.png` and `og-image-it.png`), schema.org `ProfessionalService` structured data, `sitemap.xml`, `robots.txt` and an inline SVG favicon.
- 🌍 **Slovak, Czech, Hungarian, Polish, English, German and Italian on one domain** - the Czech version lives in `/cs/` (rate 25 CZK/€), the Hungarian one in `/hu/` (rate 400 HUF/€), the Polish one in `/pl/` (rate 4.35 PLN/€), the English one in `/en/` (written for Ireland, prices stay in euro), the German one in `/de/` (written for Germany and Austria, prices stay in euro) and the Italian one in `/it/` (written for Italy, prices stay in euro). Each includes all 44 demos, with prices converted and local facts in place of Slovak ones. The language switcher sits in the header, search engines get `hreflang` in the head and in `sitemap.xml`. The Slovak files stay the source of truth - the translations are generated from them, so the versions cannot drift apart.
...
│                                 # gallery, 404 and privacy page (65 KB)
├── script.js                     # shared script (23 KB)
├── ukazky/                       # "ukazky" = demos
...
├── hu/                           # Hungarian version of the whole site, all 44 demos
│   ├── index.html                # the same page in Hungarian
│   ├── ukazky/                   # gallery and all 44 demos in Hungarian
│   ├── 404.html
│   └── ochrana-osobnych-udajov.html
├── pl/                           # Polish version of the whole site, all 44 demos
│   ├── index.html                # the same page in Polish
│   ├── ukazky/                   # gallery and all 44 demos in Polish
│   ├── 404.html
│   └── ochrana-osobnych-udajov.html
├── en/                           # English version of the whole site, all 44 demos (for Ireland)
│   ├── index.html                # the same page in English
...
│   ├── 404.html
│   └── ochrana-osobnych-udajov.html
├── it/                           # Italian version of the whole site, all 44 demos (Italy)
│   ├── index.html                # the same page in Italian
│   ├── ukazky/                   # gallery and all 44 demos in Italian
│   ├── 404.html
│   └── ochrana-osobnych-udajov.html
...
├── og-image-it.png               # the same for the Italian version
├── sitemap.xml                   # sitemap (all seven languages + hreflang)
